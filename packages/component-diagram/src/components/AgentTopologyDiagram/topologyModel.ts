...
// A sequence step and a delegation between the same two agents bow apart so both stay visible.
function spreadParallelEdges(edges: TopologyEdge[]): void {
    ...
    groups.forEach((group) => group.forEach((edge, index) => (edge.bow = index - (group.length - 1) / 2)));
}
