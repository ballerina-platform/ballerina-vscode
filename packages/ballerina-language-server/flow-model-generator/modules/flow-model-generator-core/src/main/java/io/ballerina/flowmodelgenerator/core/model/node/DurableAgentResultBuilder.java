...
import static io.ballerina.flowmodelgenerator.core.Constants.Workflow.AGENT_GET_RESULT_METHOD_NAME;
import static io.ballerina.flowmodelgenerator.core.Constants.Workflow.AGENT_RESULT_DESCRIPTION;
import static io.ballerina.flowmodelgenerator.core.Constants.Workflow.AGENT_RESULT_LABEL;
import static io.ballerina.flowmodelgenerator.core.Constants.Workflow.AGENT_WAIT_RESULT_METHOD_NAME;
 ...
 * @since 1.8.0
 */
public class DurableAgentResultBuilder extends FunctionCall {

    public static final String AGENT_KEY = "agent";
    public static final String AGENT_LABEL = "Durable Agentic Workflow";
    public static final String AGENT_DOC = "The durable agent to read the result from";
    ...
    public static final String WAIT_KEY = "waitForResult";
    public static final String WAIT_LABEL = "Wait for Result";
    public static final String WAIT_DOC = "Wait until the instance finishes (blocking). Uncheck to read "
            + "without waiting: while the instance is still working a workflow:AgentBusyError is returned - "
            + "check back later.";

    private static final String STRING_TYPE = "string";
    private static final String DEFAULT_RESULT_VAR = "agentResult";
    private static final String DEFAULT_RESULT_TYPE = "string";

    @Override
    public void setConcreteConstData() {
        metadata().label(AGENT_RESULT_LABEL).description(AGENT_RESULT_DESCRIPTION);
        codedata()
                .node(NodeKind.DURABLE_AGENT_RESULT)
                .org(WORKFLOW_ORG)
                .module(WORKFLOW_MODULE)
                .symbol(AGENT_WAIT_RESULT_METHOD_NAME);
    }
    ...
    public void setConcreteTemplateData(TemplateContext context) {
        ...
        properties().custom()
                .metadata()
                    .label("Result Type")
                    .description("The expected type of the agent's result")
                    .stepOut()
    ...
    public Map<Path, List<TextEdit>> toSource(SourceBuilder sourceBuilder) {
        ...
        boolean waitForResult = sourceBuilder.getProperty(WAIT_KEY)
        ...
        String method = waitForResult ? AGENT_WAIT_RESULT_METHOD_NAME : AGENT_GET_RESULT_METHOD_NAME;
        sourceBuilder.token()
