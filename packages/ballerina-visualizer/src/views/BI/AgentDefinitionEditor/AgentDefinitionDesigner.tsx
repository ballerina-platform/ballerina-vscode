...
import { Type, ServiceClassModel, ModelFromCodeRequest, FieldType, FunctionModel, NodePosition, removeStatement, EVENT_TYPE, FlowNode, NodeMetadata, ToolData, PropertyModel, ServiceClassSourceRequest, Property, DIRECTORY_MAP, LineRange, AvailableNode, unwrapBallerinaString } from "@wso2/ballerina-core";
...
function stripReceiver(raw: string): string {
    return raw.startsWith("self.") ? raw.slice("self.".length) : raw;
}
...
export function AgentDefinitionDesigner(props: AgentDefinitionDesignerProps) {
    ...
    const handleToolSubmit = async (data: ExtendedAgentToolRequest) => {
        ...
        if (!flowNode) return;
        const parentSymbol = data.selectedCodeData?.parentSymbol;
        const connection = isFunctionCall || !parentSymbol
            ? ""
            : parentSymbol.startsWith("self.") ? parentSymbol : `self.${parentSymbol}`;
        ...
        try {
            ...
            const toolNode = buildAgentToolNode(flowNode, data.toolName, data.description, connection,
                data.toolParameters, { className: agentClassModel.name, filePath: fileName }, data.includeContext);
    ...
    const role = unwrapBallerinaString((agentNode?.properties as any)?.role?.value);
    const instructions = unwrapBallerinaString((agentNode?.properties as any)?.instructions?.value);
