...
export async function checkAndRunPendingEnhancement(): Promise<void> {
    ...
    if (data.aiFeatureUsed) {
        ...
        const action = await window.showInformationMessage(
            "Migration AI enhancement was paused. You can resume it from 'WSO2 Agent Builder'.",
            "Open Copilot"
        );
        ...
        const action = await window.showInformationMessage(
            "Your migrated project is ready. Open 'WSO2 Agent Builder' to run AI enhancement — it can resolve TODOs, fix build errors, and refine tests.",
            "Open Copilot"
        );
        if (action === "Open Copilot") {
            openAIPanelWithPrompt();
...
let _selectedModelId: string = "wso2"; // default to WSO2 Agent Builder
...
async function ensureAuthenticated(): Promise<boolean> {
    ...
    const signingInMsg = { type: "content_block" as const, content: "Signing in to WSO2 Agent Builder...\n\n" };
...
/**
 * Triggers the WSO2 Agent Builder browser sign-in flow and waits until the user is
...
export async function runWizardMigrationEnhancement(): Promise<void> {
    ...
    if (!isAuthenticated) {
        eventHandler({
            type: "error",
            content: "Please sign in to WSO2 Agent Builder to use AI enhancement. Please retry the AI Enhancement step.",
        });
        return;
    }
