    ...
    private render(): void {
        ...
        switch (this.status.state) {
            case 'running':
                ...
                break;
            case 'awaiting-input':
                this.statusBarItem.text = `$(bi-ai-chat) ${copilotName()} needs your input`;
                this.statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.warningBackground');
                break;
            case 'completed':
                this.statusBarItem.text = `$(check) ${copilotName()} finished`;
                this.statusBarItem.backgroundColor = undefined;
                break;
            case 'error':
                this.statusBarItem.text = `$(error) ${copilotName()} error`;
                this.statusBarItem.backgroundColor = new vscode.ThemeColor('statusBarItem.errorBackground');
                break;
        }
        const tooltip = new vscode.MarkdownString();
        tooltip.appendMarkdown(`**${copilotName()}**${this.status.label ? ` — ${this.status.label}` : ''}\n\n`);
        tooltip.appendMarkdown(`Click to open the ${copilotName()} chat.`);
